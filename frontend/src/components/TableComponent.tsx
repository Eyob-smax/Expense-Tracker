...
export default function TableComponent({
  ...
  const colors = ["bg-white", "bg-zinc-100", "bg-amber-50", "bg-gray-100"];
  let colorIndex = 0;
        ...
        {forWhich === "expense" ? (
          <tbody className="text-[#61758A]">
            {(bodyArrays as TExpense[]).map((row) => {
              ...
              if (!dateColorMap[dateKey]) {
                dateColorMap[dateKey] = colors[colorIndex % colors.length];
                colorIndex++;
              }
              return (
                <tr
                  key={row.expense_id}
                  className={`border-b border-[#E4E7EC] ${dateColorMap[dateKey]}`}
                >
